...
//				EditorGUILayout.HelpBox("Use (Tools\\Parse Terms) to find how many times each of the Terms are used", UnityEditor.MessageType.Info);
			...
			if (ShowTools)
			{
				...
				GUILayout.BeginHorizontal();
					string newPrefix = EditorGUILayout.TextField("Sheet Prefix", mLanguageSource.SheetPrefix);
					if (newPrefix != mLanguageSource.SheetPrefix)
					{
						mProp_SheetPrefix.stringValue = newPrefix;
					}
				GUILayout.EndHorizontal();
